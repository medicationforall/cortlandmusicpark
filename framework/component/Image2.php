...
echo $count;
		...
		//			echo '"'.$tag.'"'.',';
			...
			if (count($tag) != $count)
// if the number of the array isnt the last fucking number do the normal comma thing, otherwise dont, it never hits it and i assume its cause its calling it a one instead of the proper 4 it is
			{
...
//check number of entries within array

//if number is less than the amount that is in the array do the for each loop

//if number is equal to last amount in array do the foreach loop modified

		//echo '"type":'.json_encode($this->keyType).',';
		//echo '"affinity":'.json_encode($this->affinity);
		//echo 'test2';
		echo '}';
